...
To run, example:
$ python train_classify.py config/train_darknet19_imagenet2012.py --n_worker=1
"""
...
# Dataloader
data_dir = os.path.join('data', dataset_name.strip('blank_'))
if dataset_name == 'imagenet2012':
    from dataloaders.imagenet import ImageNetConfig, ImageNetTrainDataLoader, ImageNetValDataLoader
    ...
    from dataloaders.fake import FakeConfig, FakeTrainDataLoader, FakeValDataLoader
    dataloader_args = dict(
        n_data=2,  # Tiny amount for debug
        img_h=img_h,
        img_w=img_w,
        imgs_mean=imgs_mean,
        imgs_std=imgs_std,
    ...
    dataloader_config = FakeConfig(**dataloader_args)
    dataloader_train = FakeTrainDataLoader(dataloader_config, batch_size=batch_size, num_workers=n_worker)
    dataloader_val = FakeValDataLoader(dataloader_config, batch_size=batch_size, num_workers=n_worker)
elif dataset_name == 'blank_imagenet2012':
    from dataloaders.imagenet import BlankImageNetConfig, BlankImageNetTrainDataLoader, BlankImageNetValDataLoader
    dataloader_args = dict(
        img_h=img_h,
        img_w=img_w,
        imgs_mean=imgs_mean,
